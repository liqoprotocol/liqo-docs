...
- Use [webhooks](/docs/webhooks) as your source of truth for fulfillment — they're immediate and cheap.
...
- Use [quotes](/docs/payments) to show estimated output/fees before creating a payment; quotes expire (`expiresAt`).

## Error handling

- Branch on `err.code` (canonical codes) and fall back to `err.message`. See [Error Handling](/docs/error-handling).
