...
description: API keys, environments, security
---
...
| `production` | `https://api.liqo.network` |
...
> **Domains:** Liqo uses the `liqo.network` domain across the board — API, hosted checkout, dashboard, and marketing site. Confirm your account's base URL in the dashboard.

## Security rules

- **Server-side only.** Never ship an API key to a browser, mobile app, or any client the user controls. Frontends should call *your* backend, which uses the SDK. See [examples/react](https://github.com/liqoprotocol/liqo-sdk/tree/main/examples/react).
...
If a key is missing or invalid, the API responds with an error the SDK raises as a `LiqoApiError` (e.g. `code: 'UNAUTHORIZED'`). See [Error Handling](/docs/error-handling).
