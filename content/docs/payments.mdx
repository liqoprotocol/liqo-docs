---
title: Payments
description: pay() and the payment lifecycle
---
...
See [Supported Currencies & Assets](/docs/currencies-assets) for the full `fromCurrency`/`toAsset`/`targetChain` reference.
...
`pay()` is a thin wrapper over [`checkout.sessions.create()`](/docs/checkout) — use either.
...
Track status via [webhooks](/docs/webhooks) (recommended) or by [polling the transaction](/docs/transactions).
