---
title: Transactions
description: Retrieve, poll, and the status model
---
...
> If you need the full canonical status, read it from the [webhook payload](/docs/webhooks) `data.status`, which carries the platform's exact `TransactionStatus`.
...
> **Prefer webhooks over polling** for production fulfillment — they're immediate and cheaper. Use polling for short-lived flows or as a fallback. See [Best Practices](/docs/best-practices).
