...
> `liqo.pay(...)` is a convenience wrapper that maps friendlier field names (`fromCurrency`/`toWallet`) onto this request. See [Payments](/docs/payments).
...
4. **Backend:** treat the signed [`transaction.completed` webhook](/docs/webhooks) as the source of truth; use `successUrl`/`cancelUrl` only for UX.
